...
| **Physics** | The invisible **surfaces** as thin cyan-white SDF line-art on a black sky with the Earth dimmed to ~28% — Shue magnetopause, Cairns/Fairfield bow shock, dipole L-shells L = 2…6, deforming in real time with `r₀`, `α` and the storm-dependent shell warp — **plus** a camera-synced 2D overlay of the *mechanism*: schematic field-direction glyphs colour-coded **open vs. closed** (closed = blue, returns to Earth; open = amber, connected to the IMF), the draped upstream IMF arrows, and the **two reconnection X-lines**. | Answer *"how does the energy actually get in?"* Open flux is the doorway: watch southward Bz open the dayside, the **dayside X-line** drop to the equator (it rides up to high-latitude lobe reconnection when Bz turns north), and — minutes later, on the lagged tail driver — the **near-Earth neutral line** light in the tail. That delay between the two X-lines *is* the substorm. Each marker is labelled on-screen; hover one for its governing relation. The field is an honest **schematic** (analytic dipole + draped IMF), not an MHD solution. |
